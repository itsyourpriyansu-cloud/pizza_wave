...
export default function PaymentPage() {
  ...
  if (payment.status === 'CONFIRMED' && order) return <section className="full-commerce-flow payment-page">
    ...
    <motion.div className="payment-result received" initial={{ opacity: 0, scale: .97 }} animate={{ opacity: 1, scale: 1 }}><div className="result-icon"><Check /></div><span>BACKEND VERIFIED</span><h1>PAYMENT RECEIVED ✓</h1><p>We're confirming your order with Pizza Wave…</p><div className="awaiting-card"><LoaderCircle /><div><span>AWAITING ACCEPTANCE</span><strong>The store is reviewing your paid order.</strong></div></div><div className="received-meta"><div><span>PAID</span><strong>₹{payment.amount}</strong></div><div><span>POINTS PENDING</span><strong>+{order.financialSnapshot.pointsToEarn}</strong></div></div><PrimaryButton onClick={() => navigate('/app/')}>RETURN HOME</PrimaryButton><small>This is not an order confirmation yet.</small></motion.div>
  </section>
